...
# Pilih ticker dan jenis minyak yang dianalisis
...
if not data.empty:
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(data['stock_datetime'], data['stock_change_normalized'], label=f"Stock Change ({selected_oil_type}) Normalized", color="green", marker="o", linestyle="-")
    plt.plot(data['oil_datetime'], data['price_change_normalized'], label=f"Oil Price Change ({selected_oil_type}) Normalized", color="red", marker="o", linestyle="--")
    plt.title(f"Normalized Price Trend: Stock ({selected_ticker}) vs Crude Oil({selected_oil_type})", fontsize=14)
    plt.xlabel("Datetime", fontsize=12)
    ...
    # # Scatter plot korelasi data normalisasi
